...
function get_content_id($content) {
	...
	$sql = "SELECT content_id FROM `blog` WHERE content = '$content'";
	$query = mysqli_query($dbCon, $sql);
...
function insert_article_comments($content_id, $comments, $comment_by) {
	include('core/db/db_connection.php');
	$content_id = (int)$content_id;
	$comments = "Comment: " . sanitize($comments);
	$comment_by = "Posted by: " . sanitize($comment_by) . "</br></br>";
	$sql = "UPDATE `blog` SET `comments` = '$comments', `comment_by` = '$comment_by' WHERE `content_id` = '$content_id'";
	mysqli_query($dbCon, $sql);
}
