...
// 	$sql = "UPDATE `blog` SET `comments` = '$comments', `comment_by` = '$comments' WHERE `content_id` = $content_id";
// 	mysqli_query($dbCon, $sql);
// }

function insert_comments($content_id, $update_data) { 
	include('core/db/db_connection.php');
	array_walk($update_data, 'array_sanitize');
	$update = array();
	foreach ($update_data as $field => $data) {
	...
	$sql = "UPDATE `blog` SET " . implode(', ', $update) . " WHERE `content_id` = " . $content_id; // converts the $update array into a string format
	// print_r($sql);
	// die();
	$query = mysqli_query($dbCon, $sql);
}
...
function list_articles() { // listeaza articolele existente pe site
	include('core/db/db_connection.php');
	$sql = "SELECT * FROM blog ORDER BY content_id DESC";
	$result = mysqli_query($dbCon, $sql);
	while ($row = mysqli_fetch_array($result)) {
		echo 
			"<h5 class='posted_by'>Posted by " . $row['posted_by'] . " on " . $row['date'] . "</h5>" . 
			"<h1 class='content_headers'>" . $title = $row['title'] . "</h1>" . 
			"<article>" . $content = $row['content'] . "</article><hr class='artline'>" . $row['comment_by'] . $row['comments'];
		// if (logged_in() === true) {
...
function create_captcha() { // creaza form-ul de captcha
	...
	echo  "<span class='captcha_num'>" . $num1 . ' + ' . $num2 . ' = ' ;
	echo '<input type="text" name="captcha_results" size="2">';
	echo '<input type="hidden" name=\'num1\' value=' . $num1 . '; ?>';
	echo '<input type="hidden" name=\'num2\' value=' . $num2 . '; ?></span>';
}
